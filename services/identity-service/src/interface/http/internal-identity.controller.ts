...
export class InternalIdentityController {
  ...
  async validateState(@Body() body: unknown): Promise<{
    ...
    return this.prisma.$transaction(async (tx: TransactionClient) => {
      ...
      // 1b. Verify session is bound to the claimed tenant and membership
      if (session.selectedTenantId !== selectedTenantId) {
        return { valid: false, code: 'SESSION_TENANT_MISMATCH' };
      }
      if (session.membershipId !== membershipId) {
        return { valid: false, code: 'SESSION_MEMBERSHIP_MISMATCH' };
      }
