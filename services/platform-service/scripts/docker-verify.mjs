#!/usr/bin/env node
/**
 * Docker image verification script for platform-service.
 *
 * Validates:
...
const IMAGE_TAG = 'carecareer/platform-service:gp-02-verify';
...
console.log('═══════════════════════════════════════════════════');
console.log('  CareCareer Platform Service — Docker Verification');
console.log('═══════════════════════════════════════════════════\n');
...
try {
  run(
    `docker build -f services/platform-service/Dockerfile -t ${IMAGE_TAG} --build-arg GIT_SHA=verify --build-arg BUILD_DATE=2025-01-01 .`,
    { silent: true },
  );
...
const testFixtures = run(
  `docker run --rm ${IMAGE_TAG} sh -c "find /app/dist -path '*/test/*' -o -path '*/fixtures/*' 2>/dev/null || true"`,
  { silent: true },
);
check('Test fixtures are absent from dist', testFixtures === '');
...
check('Only port 3000 exposed', exposedPorts.includes('3000'));
