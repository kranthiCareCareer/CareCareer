-- GP-09: Marketplace and Shift Requests
-- Workers request published shifts; requests go through review and confirmation
...
CREATE TABLE staffing.shift_requests (
    ...
    CONSTRAINT unique_active_request UNIQUE (tenant_id, shift_id, worker_id)
);

CREATE INDEX idx_shift_requests_shift ON staffing.shift_requests (shift_id, status);
CREATE INDEX idx_shift_requests_worker ON staffing.shift_requests (worker_id, status);
